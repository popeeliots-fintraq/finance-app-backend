...
from ..ml.scaling_logic import calculate_dynamic_baseline # Assume this calculates DMB and GMB
...
# NOTE: This should ideally come from a geo-specific table, but we hardcode a placeholder
...
class LeakageService:
    ...
    def _fetch_profile_data_and_baselines(self, reporting_period: date) -> Dict[str, Any]:
        ...
        if not salary_profile:
            # 🚨 FAILURE PREVENTION: If profile is missing, try to create a new one.
            # We assume 'User' holds the latest monthly_salary.
            user_info = self.db.query(User).filter(User.id == self.user_id).first()
            if not user_info:
                raise NoResultFound(f"User ID {self.user_id} not found.")

            # Create a minimal profile for the current period
            salary_profile = SalaryAllocationProfile(
                user_id=self.user_id,
                reporting_period=reporting_period,
                net_monthly_income=user_info.monthly_salary,
                fixed_commitment_total=Decimal("0.00"), # Needs a proper service call to calculate
            )
        ...
        return {
            ...
            "salary_profile": salary_profile # Pass the object back for update/commit
        }
    ...
    def calculate_leakage(self, reporting_period: date) -> Dict[str, Any]:
        ...
        # The DMB calculation itself should incorporate the GMB, but we ensure the
        # result doesn't push the effective *remaining* salary too low.
